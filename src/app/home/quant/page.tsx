      ...
      <div style={{ background:PNL,border:`1px solid ${BRD}`,borderRadius:8,width:520,overflow:"hidden",display:"flex",flexDirection:"column" }}>
        ...
        <div style={{ display:"flex",minHeight:280 }}>
          ...
          <div style={{ flex:1,padding:"16px 18px",overflowY:"auto" }}>
                ...
                {[["Navigation",nav,setNav],["Pane",pane,setPane]].map(([lbl,val,fn])=>(
                  <div key={String(lbl)} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10 }}>
                    ...
                    <div style={{ position:"relative" }}>
                      <select value={String(val)} onChange={e=>(fn as (v: string) => void)(e.target.value)}
            ...
            ) : <div style={{ color:DIM,fontSize:13,paddingTop:20,textAlign:"center" }}>{sects.find(s=>s.id===sect)?.label} settings</div>}
          </div>
        </div>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px",borderTop:`1px solid ${BRD}` }}>
          <div style={{ position:"relative" }}>
            <select style={{ background:PN2,border:`1px solid ${BRD}`,color:TXT,fontSize:12,padding:"5px 24px 5px 10px",borderRadius:4,cursor:"pointer",outline:"none",appearance:"none" }}><option>Template</option></select>
            <span style={{ position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",color:DIM,fontSize:10,pointerEvents:"none" }}>▾</span>
          </div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2 }}>
            ...
            {cells.map((d,i)=>(
              <div key={i} onClick={()=>d&&setSelDay(d)} style={{
                fontSize:12.5,textAlign:"center",padding:"5px 0",cursor:d?"pointer":"default",borderRadius:"50%",
...
export default function QuantTerminal() {
  ...
  // Load on symbol/tf change (deferred: avoid synchronous setState in effect)
  useEffect(() => {
  ...
  const runResearch = useCallback(async (s: string) => {
    ...
    }catch(e){if((e as Error)?.name!=="AbortError") setStatusMsg("Failed");}
          ...
          <div style={{ width:224,borderLeft:`1px solid ${BRD}`,display:"flex",flexDirection:"column",flexShrink:0,overflow:"hidden" }}>
            ...
            <div style={{ display:"flex",height:36,borderBottom:`1px solid ${BRD}`,flexShrink:0 }}>
              {(["buy","sell"] as Side[]).map(s=>(
                <button key={s} onClick={()=>setSide(s)} style={{
                  ...
                  color:side===s?(s==="buy"?G:R):DIM,fontSize:13,fontWeight:600,cursor:"pointer",marginBottom:-1,textTransform:"capitalize",
                }}>{s}</button>
              ))}
            </div>
            ...
            <div style={{ flex:1,overflowY:"auto",padding:"8px 8px" }}>
              ...
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6 }}>
                ...
                <div style={{ position:"relative" }}>
                  <select style={{ background:PN2,border:`1px solid ${BRD}`,color:TXT,fontSize:10,padding:"2px 22px 2px 6px",borderRadius:3,cursor:"pointer",outline:"none",appearance:"none" }}>
