...
function computeRisk(bars, closes) { return { realizedVolAnnualized: round((lastValue(realizedVolSeries(closes, 30)) ?? 0.2) * 100, 2), ...drawdownStats(closes) }; }
export async function generateSignalResponse(symbols) {
...
async function computeSignal(symbol, timeframe = "1Day") {
  ...
  const pct = n => closes.length > n ? round((price / closes[closes.length - 1 - n] - 1) * 100, 2) : 0;
...
async function computeRegime(benchmark = "SPY") {
  try {
