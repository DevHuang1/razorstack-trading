...
function deriveRegime(
  ...
  if (vol30 !== null && vol30 < 0.22) score++; // low vol → risk on
  if (vol30 !== null && vol30 > 0.40) score--; // high vol → risk off
...
export async function fetchMarketData(
  ...
  const volSeries = realizedVolSeries(closes, 30);
  const vol30 = lastValue(volSeries);

  const pct = (idx: number) =>
    closes.length > idx
      ? r2(((price - closes[closes.length - 1 - idx]) / closes[closes.length - 1 - idx]) * 100)
      : 0;
  ...
  const avgVol30 =
    slice30.length > 0
      ? slice30.reduce((a, b) => a + b, 0) / slice30.length
      : undefined;
  ...
  const snapshot: MarketSnapshot = {
    ...
    change1mPct: pct(21),
    rsi14: r2(rsi14),
    sma20: r2(sma20 ?? price),
    sma50: r2(sma50 ?? price),
    realizedVol30dAnnPct: r2((vol30 ?? 0.22) * 100),
...
export async function fetchNews(symbol: string): Promise<NewsItem[]> {
  ...
  if (!keyId || !secretKey) {
    // Minimal synthetic stub when no key is configured
    return [
      {
        id: `synthetic-${symbol}-1`,
        headline: `${symbol} under review amid current market conditions`,
        summary: `Analysts are monitoring ${symbol} as broader macro conditions evolve.`,
        source: "Synthetic",
  ...
  try {
    ...
    if (!res.ok) return [];
    const json = (await res.json()) as { news?: AlpacaNewsItem[] };
